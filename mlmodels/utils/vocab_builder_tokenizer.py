...
from mlmodels.utils.special_tokens import PAD, UNK, SOT, EOT, COL, TAB
from mlmodels.utils.txtdocIO import TextDataset
...
class Vocab(object):
    ...
    def build(self, files, tokenizer=None):
        """
        ...
        for fname in files:
            # Read input files
            raw = TextDataset(file_path=fname, tokenizer=tokenizer, model_type="", overwrite_cache=True, read_line=True)
            for line in raw:
                count += 1
                nl = line
                target = line
    ...
    @staticmethod
    def lst2idx(vocab_words=None, unk_words=True, sos=False, eos=False,
    ...
    @staticmethod
    def tok2id(tokenizer, block_size=512):
        """
        ...
        def f(tokens):
            ...
            tokenized_ids = tokenizer.convert_tokens_to_ids(tokens)[:block_size]
            tokenized_ids = tokenizer.build_inputs_with_special_tokens(tokenized_ids)
...
if __name__ == '__main__':
    ...
    dtype = torch.long
    use_cuda = False
    filename = "../../data/wikitext2/raw/wiki.train.raw"

    s_paras = [-1,  1]
    t_paras = [-1, 1]
    tokenizer = GPT2Tokenizer.from_pretrained("gpt2")
    vocab = Vocab(s_paras, t_paras)
    vocab.build([filename], tokenizer.tokenize)
    ...
    train_data = TextDataset(filename, source2idx=nl2ids, overwrite_cache=False)
    #
    # tg2ids = vocab.lst2idx(vocab_words=vocab.tw2i, unk_words=False, sos=True, eos=True)
    #
    # train_data = TextDataset(filename, source2idx=nl2ids, target2idx=tg2ids)
