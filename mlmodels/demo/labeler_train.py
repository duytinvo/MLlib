...
"""
import sys
import mlmodels.demo.labeler_settings as settings
from mlmodels.training.labeler_model import main
...
if __name__ == '__main__':
    ...
    python -m mlmodels.demo.labeler_train --use_cuda --label_file /media/data/disambiguator/corpus/labels.txt --train_file /media/data/disambiguator/corpus/train.csv --dev_file /media/data/disambiguator/corpus/dev.csv --test_file /media/data/disambiguator/corpus/test.csv --pred_test_file /media/data/disambiguator/regression/test.csv --model_dir /media/data/disambiguator/traned_model/ --tokenize_type splitter --timestamped_subdir
    screen python -m mlmodels.demo.labeler_train --use_cuda --label_file /media/data/disambiguator/corpus/labels.txt --train_file /media/data/disambiguator/corpus/train.csv --dev_file /media/data/disambiguator/corpus/dev.csv --test_file /media/data/disambiguator/corpus/test.csv --pred_test_file /media/data/disambiguator/regression/test.csv --model_dir /media/data/disambiguator/traned_model/ --tokenize_type splitter --timestamped_subdir --swd_embfile /media/data/embeddings/glove/glove.42B.300d.txt --ed_mode lstm --decay_rate 0.05 --clip 5 --ed_layers 2
    """
    ...
    if '--use_crf' not in sys.argv:
        if settings.use_crf:
            sys.argv.append('--use_crf')
    if '--se_transitions' not in sys.argv:
        if settings.se_transitions:
            sys.argv.append('--se_transitions')
