...
from mlmodels.training.trans_labeler_model import main
...
if __name__ == '__main__':
    ...
    if '--overwrite_cache' not in sys.argv:
    ...
    if '--config_name' not in sys.argv:
        sys.argv.extend(['--config_name', settings.config_name])
    if '--tokenizer_name' not in sys.argv:
    ...
    # if '--do_lower_case' not in sys.argv:
    #     if settings.do_lower_case:
    #         sys.argv.append('--do_lower_case')
    # if '--keep_accents' not in sys.argv:
    #     if settings.keep_accents:
    #         sys.argv.append('--keep_accents')
    # if '--strip_accents' not in sys.argv:
    #     if settings.strip_accents:
    #         sys.argv.append('--strip_accents')
    if '--use_fast' not in sys.argv:
        if settings.use_fast:
            sys.argv.append('--use_fast')
    ...
    # Training setup
    if '--max_seq_length' not in sys.argv:
        sys.argv.extend(['--max_seq_length', str(settings.max_seq_length)])
    ...
    # if '--server_ip' not in sys.argv:
    #     sys.argv.extend(['--server_ip', settings.server_ip])
    # if '--server_port' not in sys.argv:
    #     sys.argv.extend(['--server_port', settings.server_port])
