...
from mlmodels.metrics.prf1 import NER_metrics, APRF1
...
from mlmodels.modules.labeler import Labeler
...
class Labeler_model(object):
    def __init__(self, args=None):
        ...
        crf_HPs = [self.args.use_crf, self.num_labels, self.args.se_transitions]
        print("INFO: - Build model...")
        self.labeler = Labeler(nlemb_HPs, enc_HPs, crf_HPs, drop_rate=self.args.final_dropout, num_labels=self.num_labels)
    ...
    def init_optimizers(self, opt_method=optim.SGD):
        self.labeler_optimizer = opt_method(self.labeler.parameters(), lr=self.args.lr)

    def predict_batch(self, entries, wombat_object=None, return_probability=False):
        nl = []
        wd_tokens = []
        ...
        with torch.no_grad():
            ...
            de_score = self.labeler(nl_tensor, nl_len_tensor, wombat_tensor=wombat_tensor)
            label_mask = nl_tensor > 0
            if return_probability is False:
                output_prob, output_idx = self.labeler.inference(de_score, label_mask)
                if self.args.use_crf:
                    predict_words = Tokenizer.decode_batch(output_idx, self.tokenizer.i2tw, 2)
                    ...
                    predict_words = Tokenizer.decode_batch(output_idx.squeeze(-1).tolist(), self.tokenizer.i2tw, 2)
                    predict_words = [words[:i] for words, i in zip(predict_words, label_mask.sum(dim=1).tolist())]
                    # predict_prob = acc_prob.prod(dim=-1).tolist()
                    predict_prob = [words[:i] for words, i in
                                    zip(output_prob.squeeze(-1).tolist(), label_mask.sum(dim=1).tolist())]
                ...
                for i, entry in enumerate(entries):
                    ...
                    entities_list = NER_metrics.absa_extractor(entry["input_list"], predict_words[i],
                    ...
                    if len(entities_list) > 0:
                        for entity, senti, _, prob in entities_list:
                            ...
                            entry["entities"].append({"aspect": entity, "polarity": senti, "probability": prob})

                return entries
            else:
                label_prob = torch.softmax(de_score.squeeze(), dim=-1)
                return [{self.tokenizer.i2tw[ind]: prob for ind, prob in enumerate(prob_i)}
    ...
    def evaluate_batch(self, eva_data, num_eva):
        ...
        with torch.no_grad():
            for i, d in enumerate(eva_dataloader):
                ...
                total_loss = self.labeler.NLL_loss(de_score, lb_tensor, label_mask)

                dev_loss.append(total_loss.item())
                total_tokens += label_mask.sum()
                output_prob, output_idx = self.labeler.inference(de_score, label_mask)

                label_words = Tokenizer.decode_batch(lb_tensor.tolist(), self.tokenizer.i2tw, 2)
                ...
                if self.args.use_crf:
                    ...
                    predict_words = [words[:i] for words, i in zip(predict_words, label_mask.sum(dim=1).tolist())]
                    # predict_prob = acc_prob.prod(dim=-1).tolist()
                    predict_probs += [words[:i] for words, i in
                                      zip(output_prob.squeeze(-1).tolist(), label_mask.sum(dim=1).tolist())]
    ...
    def train_batch(self, train_data, num_train):
        ...
        for i, d in enumerate(train_dataloader):
            ...
            if clip_rate > 0:
                torch.nn.utils.clip_grad_norm_(self.labeler.parameters(), clip_rate)
            # update parameters in all sub-graphs
            self.labeler_optimizer.step()
    ...
    def train(self):
        ...
        if self.args.tl:
            ...
            print("         - Current Dev sep_acc: %.4f; Current Dev full_acc: %.4f" % (dev_metrics[3], dev_metrics[4]))
            self.appendfile("\t- Transfer learning performance")
            self.appendfile("\t\t- Current Dev loss: %.4f; Current Dev P: %.4f; Current Dev R: %.4f; "
                            "Current Dev F1: %.4f; Dev speed: %.2f(tokens/s)" %
                            (dev_loss, dev_metrics[0], dev_metrics[1], dev_metrics[2], dev_speed))
            self.appendfile("\t\t- Current Dev sep_acc: %.4f; Current Dev full_acc: %.4f" %
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            if cond:
                ...
                print("         - Current Dev loss: %.4f; Current Dev P: %.4f; Current Dev R: %.4f; "
                      "Current Dev F1: %.4f; Dev speed: %.2f(tokens/s)" %
                      (dev_loss, dev_metrics[0], dev_metrics[1], dev_metrics[2], dev_speed))
                print("         - Current Dev sep_acc: %.4f; Current Dev full_acc: %.4f" %
                      (dev_metrics[3], dev_metrics[4]))
                ...
                if nepoch_no_imprv >= self.args.patience:
                    ...
                    print("         - Current Test sep_acc: %.4f; Current Test full_acc: %.4f" %
                    ...
                    self.appendfile("\t\t- Test loss: %.4f; Test P: %.4f; Test R: %.4f; "
                                    "Test F1: %.4f; Test speed: %.2f(tokens/s)\n" %
                                    (test_loss, test_metrics[0], test_metrics[1], test_metrics[2], test_speed))
                    self.appendfile("\t\t- Current Test sep_acc: %.4f; Current Test full_acc: %.4f" %
                                    (test_metrics[3], test_metrics[4]))
                    return test_metrics
    ...
    def save_parameters(self, epoch=1):
        ...
        labeler_filename = os.path.join(epoch_dir, self.args.labeler_file)
        print("\t- Save model to file: %s" % labeler_filename)
        self.labeler.to("cpu")
        torch.save(self.labeler.state_dict(), labeler_filename)
        self.labeler.to(self.device)
    ...
    def load_parameters(self, epoch=1):
        ...
        self.labeler.load_state_dict(torch.load(labeler_filename))
        self.labeler.to(self.device)
    ...
    def load_transferlearning(self, epoch=-1):
        ...
        try:
            ...
            print("\t- UNABLE to load model from file: %s" % self.args.labeler_file)

    def lr_decay(self, epoch):
        lr = self.args.lr / (1 + self.args.decay_rate * epoch)
        print("INFO: - No improvement; Learning rate is setted as: %f" % lr)
        for param_group in self.labeler_optimizer.param_groups:
            param_group['lr'] = lr

    @staticmethod
    def class_metrics(reference, candidate):
        P, R, F1 = NER_metrics.span_metrics(reference, candidate)
        sep_acc, full_acc = APRF1.accuracies(reference, candidate)
        return P, R, F1, sep_acc, full_acc
    ...
    def build_data(args):
        ...
        if not args.tl:
            ...
            args.labeler_file = margs.labeler_file
...
def main(argv):
    ...
    argparser.add_argument('--model_args', help='Trained argument filename', default="labeler.args", type=str)
    argparser.add_argument('--labeler_file', help='Trained labeler_file filename',
                           default="labeler.m", type=str)
    ...
    argparser.add_argument("--use_crf", action='store_true', default=False, help="CRF Flag (default False)")
    argparser.add_argument("--se_transitions", action='store_true', default=False, help="start-end transitions")
    ...
    args = Labeler_model.build_data(args)

    translator = Labeler_model(args)
...
if __name__ == '__main__':
    """
    python labeler_model.py --use_cuda --teacher_forcing_ratio 0.8
    """
    main(sys.argv)
